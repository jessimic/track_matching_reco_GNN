...
class Mx2Data:
    ...
    def process_entries(self):
        ...
        for entry in range(nentries):
            ...
            fail = self.process_one_entry(entry)
            if not fail:
                # set the entry id
    ...
    def process_one_entry(self,entry):
        ...
        #Count tracks, including if any are split over up & downstream in the event
        total_groups = len(self.trk_index[entry]) 
        for idx in self.trk_index[entry]:
            n_nodes = self.trk_nodes[entry][idx]
            if ((n_nodes >0)):
                try:
                    unique_id = self.get_most_energetic_particle(entry, idx)
                except:
                    return 1
                track_ids_unique.append(unique_id)
                group_ids.append(idx)
                fragment_ids.append(total_fragments)
                total_fragments += 1
                z_nodes = Mx2Hits.trk_node_Z[entry][idx][:n_nodes] - offsetZ
                any_us = sum(z_nodes < 0) > 0
                any_ds = sum(z_nodes > 0) > 0
                if any_us and any_ds:
                    group_ids.append(idx)
                    fragment_ids.append(total_fragments)
                    total_fragments += 1
        if len(set(track_ids_unique)) != len(track_ids_unique):
            return 1
        id_shuffle = np.random.permutation(total_fragments)
        fa = np.array(fragment_ids)
        fragment_ids_shuffled = fa[id_shuffle]
